...
RETURNS TEXT AS $$
BEGIN
    RETURN (SELECT dns_zone FROM public.exhibitions WHERE id = exhibition_id);
END;
$$ LANGUAGE plpgsql;
...
-- Create triggers for the hosts table
CREATE TRIGGER sync_host_dns
    AFTER INSERT OR UPDATE OR DELETE ON public.hosts
    FOR EACH ROW
