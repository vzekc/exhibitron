...
const RegistrationDetails = () => {
  ...
  const { data, refetch } = useQuery(GET_REGISTRATION, { variables: { id: parseInt(id) } })
  ...
  const handleStatusChange = async (status: string) => {
    switch (status) {
      case 'approved':
        await approveRegistration({ variables: { id: parseInt(id), siteUrl: window.location.origin } })
        break
      case 'rejected':
            ...
            <div className="grid">
              ...
              {registration.status !== 'approved' && (
                <button type="button" onClick={() => setConfirmAction({
                  ...
                  actionName: 'Löschen',
                  action: async () => {
                    await deleteRegistration({ variables: { id: parseInt(id) } })
