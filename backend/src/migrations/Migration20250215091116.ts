...
export class Migration20250215091116 extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      `create table "user" ("id" serial primary key, "created_at" timestamptz not null, "updated_at" timestamptz not null, "full_name" varchar(255) not null, "email" varchar(255) not null, "password" varchar(255) not null, "bio" text not null default '', "social" jsonb null);`,
    )

    this.addSql(
      `create table "article" ("id" serial primary key, "created_at" timestamptz not null, "updated_at" timestamptz not null, "slug" varchar(255) not null, "title" varchar(255) not null, "description" varchar(1000) not null, "text" text not null, "author_id" int not null);`,
    )
    this.addSql(
    ...
    this.addSql(
      `create table "comment" ("id" serial primary key, "created_at" timestamptz not null, "updated_at" timestamptz not null, "text" varchar(1000) not null, "article_id" int not null, "author_id" int not null);`,
    )

    this.addSql(
      `create table "article_tags" ("article_id" int not null, "tag_id" int not null, constraint "article_tags_pkey" primary key ("article_id", "tag_id"));`,
    )
